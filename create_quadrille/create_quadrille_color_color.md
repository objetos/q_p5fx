...
Creates an 8x8 quadrille with a chessboard pattern using the given colors — `color1` for the light squares, `color2` for the dark.
...
{{< callout type="info" >}}
`createQuadrille(color1, color2)` is equivalent to `createQuadrille(8, 8).fill(color1, color2)`. See [createQuadrille(width, height)]({{< ref "create_quadrille_width_height" >}}) and [fill(color1, color2)]({{< ref "fill_color_color" >}}).
{{< /callout >}}
